//! Pico 2 W gun: trigger press sends a NEC IR burst. Optional USB serial debug mode.
...
static PICOTOOL_ENTRIES: [embassy_rp::binary_info::EntryAddr; 4] = [
    embassy_rp::binary_info::rp_program_name!(c"gun"),
    embassy_rp::binary_info::rp_cargo_version!(),
    embassy_rp::binary_info::rp_program_description!(c"Pico 2 W laser-tag gun + OLED"),
...
async fn run_normal(
    ...
    loop {
        ...
        .await
        {
            ...
            Either3::Third(()) => {
                if line.as_str() == "ready" || line.as_str() == "btn short?" {
                    line = idle_line();
                }
                screen.show(line.as_str(), btn_status(), *presses, btn_high());
            }
...
fn idle_line() -> String<12> {
    ...
    let _ = line.push_str(if btn_high() { "ready" } else { "btn short?" });
    line
}
