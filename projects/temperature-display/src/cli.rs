...
pub fn start(spawner: Spawner, usb: Peri<'static, USB>, flash: ConfigFlash) {
    ...
    config.product = Some("Pico 2 W temp");
    ...
    match cmd {
        ...
        "server" => {
            if settings::parse_server(rest).is_none() {
                let _ = write_text(
                    class,
                    "usage: server <host:port/path>\r\n  example: 192.168.1.10:8080/temp\r\n",
                )
                .await;
                return;
            }
